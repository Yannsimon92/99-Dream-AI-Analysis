...
function ComposeScreen({ onBack, onAnalyse, accent }) {
  const [text, setText] = useSt('');
  // auto-type the sample to feel alive
  useEf(() => {
    let i = 0; setText('');
    ...
    <div className="app-screen">
      ...
      <div className="app-body" style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="compose-field">
          {text}{text.length < SAMPLE.length && <span className="compose-caret" />}
          {text.length === 0 && <span className="ph">Racontez votre rêve, librement…</span>}
        </div>
        ...
        <button className="app-cta" style={{ background: accent }} onClick={onAnalyse}>
          Analyser ce rêve
        </button>
      </div>
    </div>
